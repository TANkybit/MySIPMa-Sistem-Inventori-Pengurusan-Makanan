...
<head>
  ...
  <link href="<?php echo e(asset('css/design.css')); ?>?v=4" rel="stylesheet">
<style>
  ...
  <header id="header" class="header d-flex align-items-center sticky-top" style="background: rgba(2,2,4,0.8); backdrop-filter: blur(10px); border-bottom: 1px solid rgba(255,255,255,0.05);">
    <div class="container position-relative d-flex align-items-center justify-content-between">
      ...
      <div class="d-none d-xl-flex align-items-center gap-3">
        ...
        <button type="button" class="btn btn-custom btn-logout btn-sm px-3 py-2 <?php echo e(in_array(strtoupper(Auth::user()->getPositionCode()), ['PS','PR','PP'], true) ? 'logout-icon-only' : ''); ?>" id="desktopLogoutBtn" title="Log Keluar" aria-label="Log Keluar"><i
              class="bi bi-box-arrow-right me-2"></i><span class="logout-label">Log Keluar</span></button>
      </div>
    </div>
  </header>
  <main class="main">
...
unset($__errorArgs, $__bag); ?>" id="musterTolakParol" name="muster_ditolak_parol" type="number" min="0" step="1" value="<?php echo e(old('muster_ditolak_parol', $inden->muster_ditolak_parol ?? 0)); ?>" readonly required>
...
unset($__errorArgs, $__bag); ?>
          ...
          <div class="col-md-3">
            <label class="form-label">Banduan Biasa</label>
            <input class="form-control muster-input diet-muster-input" id="dietMusterStandard" name="diet_musters[standard]" data-diet-code="standard" type="number" min="0" step="1" value="<?php echo e(old('diet_musters.standard', $dietMusters['standard'] ?? ($inden->muster_ditolak_parol ?? 0))); ?>" readonly>
            <small class="text-muted">Baki automatik selepas kategori khas.</small>
          </div>
          <input id="musterKhas" name="muster_khas_daging" type="hidden" value="0">
          <input id="musterExclusion" type="hidden" value="0">
          <div class="col-12">
            <h3 class="h6 mb-1">Pecahan Kategori Skala Diet</h3>
            <p class="small text-muted mb-3">Masukkan hanya kategori yang berkenaan. Banduan Biasa dikira secara automatik.</p>
            <div class="row g-3">
              <?php $__currentLoopData = ($dietCategories ?? collect())->where('code', '!=', 'standard'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dietCategory): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="col-md-4 col-lg-3">
                  <label class="form-label" for="dietMuster_<?php echo e($dietCategory->code); ?>"><?php echo e($dietCategory->name); ?></label>
                  <input class="form-control muster-input diet-muster-input"
                         id="dietMuster_<?php echo e($dietCategory->code); ?>"
                         name="diet_musters[<?php echo e($dietCategory->code); ?>]"
                         data-diet-code="<?php echo e($dietCategory->code); ?>"
                         type="number" min="0" step="1"
                         value="<?php echo e(old('diet_musters.' . $dietCategory->code, $dietMusters[$dietCategory->code] ?? 0)); ?>"
                         <?php echo e($fieldState); ?>>
                </div>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
          </div>
          <div class="col-12">
            <div id="musterWarnings" class="d-none" style="margin-top:8px;">
              ...
              <div id="musterWarnKhas" class="d-none" style="color:#f59e0b;font-size:.85rem;font-weight:600;margin-top:4px;"><i class="bi bi-exclamation-triangle-fill me-1"></i>Jumlah kategori diet melebihi Muster Ditolak Parol.</div>
            </div>
          </div>
        </div>
      ...
      <div class="card-box section-card mb-4">
        ...
        <div class="items-wrap">
          ...
          <div id="dietContractWarning" class="alert alert-warning border-0 rounded-3 mb-0 mx-3 mt-3 d-none" style="font-size:.85rem;">
            <i class="bi bi-exclamation-triangle-fill me-1"></i>
            <strong>Item menu belum dipadankan dengan kontrak ini (semak jika bekalan dalaman):</strong>
            <span id="dietContractWarningItems"></span>
          </div>
          <div id="dietMenuInfo" class="alert border-0 rounded-3 mb-0 mx-3 mt-3 d-none" style="font-size:.85rem;background:rgba(14,165,233,.12);color:#7dd3fc;">
            <i class="bi bi-calendar-check me-1"></i><strong>Menu rasmi:</strong>
            <span id="dietMenuInfoItems"></span>
          </div>
          <div class="p-3">
...
unset($__errorArgs, $__bag); ?>
      ...
      <div class="action-row">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-center">
          ...
          <button class="btn btn-round btn-soft" type="button" onclick="window.open('<?php echo e(route('borang.inden.cetak', ['order' => $inden->inden_id])); ?>', '_blank')">Cetak Borang Inden</button>
          <?php endif; ?>
          <?php if (! ($isReadOnly)): ?>
...
<?php if(false && $canManageMusterRates && !$isReadOnly): ?>
...
<template id="itemTemplate">
  <tr class="item-card">
    ...
    <td data-order="0">
      ...
      <input class="form-control item-order-qty item-calc" type="number" min="0" step="0.001" value="0" style="display:none;">
    </td>
    <td data-order="0">
  ...
  <script>
    (function () {
      ...
      const savedDietSuggestions = <?php echo json_encode($dietSnapshotSuggestions ?? [], 15, 512) ?>;
      const savedDietMenus = <?php echo json_encode($dietSnapshotMenus ?? [], 15, 512) ?>;
      const savedDietMissing = <?php echo json_encode($dietSnapshotMissing ?? [], 15, 512) ?>;
      ...
      function updateMusterSummary(event) {
        ...
        try {
          ...
          const classifiedTotal = Array.from(document.querySelectorAll('.diet-muster-input:not([data-diet-code="standard"])'))
            .reduce((total, input) => total + numberValue(input), 0);
          const standardEl = document.getElementById('dietMusterStandard');
          if (standardEl) standardEl.value = Math.max(ditolak - classifiedTotal, 0);
          khas = classifiedTotal;
          khasEl.value = 0;
          exclusion = Math.max(ditolak - classifiedTotal, 0);
      ...
      function refreshPrismSuggestions() {
        const contractWarning = document.getElementById('dietContractWarning');
        ...
        // Submitted Inden use the frozen calculation snapshot. This is what
        // Penerima/Pengesah audit, regardless of later guideline revisions.
        if (isReadOnly && Object.keys(savedDietSuggestions).length > 0) {
          const warningItems = document.getElementById('dietContractWarningItems');
          if (contractWarning && warningItems) {
            warningItems.textContent = savedDietMissing.join(', ');
            contractWarning.classList.toggle('d-none', savedDietMissing.length === 0);
          }
          const menuInfo = document.getElementById('dietMenuInfo');
          const menuInfoItems = document.getElementById('dietMenuInfoItems');
          const menuLines = Object.entries(savedDietMenus).map(([category, dishes]) => category + ': ' + dishes.join(', '));
          if (menuInfo && menuInfoItems) {
            menuInfoItems.textContent = menuLines.join(' | ');
            menuInfo.classList.toggle('d-none', menuLines.length === 0);
          }
          getItemRows().forEach(card => {
            const suggestion = savedDietSuggestions[card.querySelector('.item-contract-id').value];
            const suggestionEl = card.querySelector('.item-muster-suggestion');
            if (!suggestion || !suggestionEl) return;
            card.dataset.musterSuggestedQty = suggestion.quantity;
            suggestionEl.innerHTML = '<strong>' + formatNumber(suggestion.quantity) + '</strong><br>'
              + '<span style="font-size:.75rem">' + (suggestion.calculation || '') + '<br>'
        ...
        const params = new URLSearchParams({
          ...
          muster_ditolak_parol: normalMuster,
          muster_khas_daging: specialMuster
        });
        document.querySelectorAll('.diet-muster-input').forEach(input => {
          params.append('diet_musters[' + input.dataset.dietCode + ']', input.value || 0);
        });
        prismSuggestionRequest?.abort?.();
        prismSuggestionRequest = new AbortController();
        fetch('<?php echo e(url("borang-inden/contract-items")); ?>/' + contractId + '/prism-suggestions?' + params, { signal: prismSuggestionRequest.signal })
          .then(response => response.ok ? response.json() : Promise.reject())
          .then(data => {
            const contractWarningItems = document.getElementById('dietContractWarningItems');
            const missingItems = data.required_items_missing_contract || [];
            if (contractWarning && contractWarningItems) {
              contractWarningItems.textContent = missingItems.join(', ');
              contractWarning.classList.toggle('d-none', missingItems.length === 0);
            }
            const menuInfo = document.getElementById('dietMenuInfo');
            const menuInfoItems = document.getElementById('dietMenuInfoItems');
            const menuLines = Object.entries(data.menus || {}).map(([category, dishes]) => category + ': ' + dishes.join(', '));
            if (menuInfo && menuInfoItems) {
              menuInfoItems.textContent = menuLines.join(' | ');
              menuInfo.classList.toggle('d-none', menuLines.length === 0);
            }
      ...
      (function wireMusterRateSettings() {
        ...
        saveButton.addEventListener('click', function () {
          ...
          fetch('<?php echo e(url("borang-inden/contract-items")); ?>/' + item.id + '/muster-rate', {
            method: 'PUT',
      ...
      function collectDietMusters() {
        const values = {};
        document.querySelectorAll('.diet-muster-input').forEach(input => {
          values[input.dataset.dietCode] = input.value || 0;
        });
        return values;
      }
      ...
      function collectFormData() {
        return {
          ...
          diet_musters: collectDietMusters(),
      ...
      function loadContractsAndRestore(contractId, items) {
        ...
        fetch(url)
          ...
          .then(function (contracts) {
            ...
            if (selId) {
              fetch('<?php echo e(url("borang-inden/contract-items")); ?>/' + selId)
                .then(function (r) { return r.json(); })
                .then(function (res) {
                  contractCeilingRemaining = res.ceiling_remaining;
                  contractItems = (res.items || []).map(ci => ({
      ...
      function restoreDraft(data) {
        ...
        Object.entries(data.diet_musters || {}).forEach(function ([code, value]) {
          var input = document.querySelector('.diet-muster-input[data-diet-code="' + code + '"]');
