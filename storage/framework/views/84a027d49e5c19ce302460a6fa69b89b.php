...
<head>
  ...
  <style>
    ...
    #inventStatsRow .stat-card.card-filter-active { box-shadow: 0 0 0 3px var(--accent), 0 25px 70px rgba(16,185,129,.35); transform: translateY(-6px); }
    #inventStatsRow .stat-card.card-filter-active::before { opacity: 1; }
    #inventStatsRow .stat-card.card-filter-active .stat-title { color: var(--accent); }
  ...
  <header id="header" class="header d-flex align-items-center sticky-top" style="background: rgba(2,2,4,0.8); backdrop-filter: blur(10px); border-bottom: 1px solid rgba(255,255,255,0.05);">
    <div class="container position-relative d-flex align-items-center justify-content-between">
      ...
      <div class="d-none d-xl-flex align-items-center gap-3">
        ...
        <button type="button" class="btn btn-custom btn-logout btn-sm px-3 py-2 <?php echo e(in_array(strtoupper(Auth::user()->getPositionCode()), ['PS','PR','PP'], true) ? 'logout-icon-only' : ''); ?>" id="desktopLogoutBtn" title="Log Keluar" aria-label="Log Keluar"><i class="bi bi-box-arrow-right me-2"></i><span class="logout-label">Log Keluar</span></button>
      </div>
    </div>
  </header>
  ...
  <main class="dashboard-container">
    <div class="container" data-aos="fade-up">
      ...
      <div class="row g-4 mb-5 justify-content-center" id="inventStatsRow">
        <div class="col-md-3">
          <div class="stat-card text-center" id="cardJumlah" data-filter="all" role="button" tabindex="0" style="cursor:pointer;">
        ...
        <div class="col-md-3">
          <div class="stat-card text-center" id="cardHampirHabis" data-filter="hampir" role="button" tabindex="0" style="cursor:pointer;">
            <div class="glow-ring"></div>
        ...
        <div class="col-md-3">
          <div class="stat-card text-center" id="cardSederhana" data-filter="sederhana" role="button" tabindex="0" style="cursor:pointer;">
            <div class="glow-ring"></div>
        ...
        <div class="col-md-3">
          <div class="stat-card text-center" id="cardBanyakLagi" data-filter="banyak" role="button" tabindex="0" style="cursor:pointer;">
            <div class="glow-ring"></div>
      ...
      <div class="row justify-content-center mb-3">
        <div class="col-lg-3 d-flex align-items-center gap-2">
          <label for="categoryFilter" class="form-label mb-0" style="white-space:nowrap;">Kategori:</label>
              ...
              <table id="inventoriTable" class="table table-dark-custom table-inventori w-100">
                ...
                <tbody>
                  <?php $__empty_1 = true; $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr data-category="<?php echo e($item->category_name ?? ''); ?>" data-status="<?php echo e($item->status); ?>">
  ...
  <script>
    $(document).ready(function() {
        ...
        $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
            ...
            var statusFilter = $('#inventStatsRow').attr('data-filter') || 'all';
            var row = inventTable.row(dataIndex).node();
            if (cat && $(row).data('category') !== cat) return false;
            if (statusFilter !== 'all') {
                var rowStatus = $(row).data('status') || '';
                var allowed = {
                    'hampir': ['Habis', 'Hampir Habis'],
                    'sederhana': ['Sederhana'],
                    'banyak': ['Banyak Lagi', 'Tiada Kontrak']
                }[statusFilter] || [];
                if (allowed.indexOf(rowStatus) === -1) return false;
            }
            return true;
        });
        ...
        // Card click filter
        $('#inventStatsRow .stat-card').on('click', function() {
            var filter = $(this).data('filter') || 'all';
            var isActive = $('#inventStatsRow').attr('data-filter') === filter;
            $('#inventStatsRow').attr('data-filter', isActive ? 'all' : filter);
            $('#inventStatsRow .stat-card').removeClass('card-filter-active');
            if (!isActive) $(this).addClass('card-filter-active');
            inventTable.draw();
        });
        <?php endif; ?>
    });
