      ...
      <div class="card-box section-card mb-4">
        ...
        <div class="row g-4">
          ...
          <div class="col-md-3">
            ...
            <input class="form-control muster-input diet-muster-input" id="dietMusterStandard" name="diet_musters[standard]" data-diet-code="standard" type="number" min="0" step="1" value="{{ old('diet_musters.standard', $dietMusters['standard'] ?? ($inden->muster_ditolak_parol ?? 0)) }}" readonly>
          ...
          <div class="col-12">
            ...
            <div class="row g-3">
              @foreach(($dietCategories ?? collect())->where('code', '!=', 'standard') as $dietCategory)
                <div class="col-md-4 col-lg-3">
                  <label class="form-label" for="dietMuster_{{ $dietCategory->code }}">{{ $dietCategory->name }}</label>
                  <input class="form-control muster-input diet-muster-input"
                         id="dietMuster_{{ $dietCategory->code }}"
                         name="diet_musters[{{ $dietCategory->code }}]"
                         data-diet-code="{{ $dietCategory->code }}"
                         type="number" min="0" step="1"
                         value="{{ old('diet_musters.' . $dietCategory->code, $dietMusters[$dietCategory->code] ?? 0) }}"
                         {{ $fieldState }}>
                </div>
              @endforeach
            </div>
...
<template id="itemTemplate">
  ...
  <script>
    (function () {
      ...
      const savedDietSuggestions = @json($dietSnapshotSuggestions ?? []);
      const savedDietMenus = @json($dietSnapshotMenus ?? []);
      const savedDietMissing = @json($dietSnapshotMissing ?? []);
